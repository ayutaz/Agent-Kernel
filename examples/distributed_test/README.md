# はじめに
この例では、Agent-Kernel の分散版（Ray ベース）を使ったマルチエージェントシステムの構築方法を示します。実行フローの理解と将来の拡張を容易にするために設計されています。
...
1. 以下のいずれかの方法で API キーを設定します:
    ...
    **`examples/distributed_test/configs/models_config.yaml`** を編集し、`api_key` フィールドを追加します:
    ```yaml
    - name: OpenAIProvider
      model: gpt-4o-mini

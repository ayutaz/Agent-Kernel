...
class SurveyInvokePlugin(InvokePlugin):
    """Execute planned actions for disaster response survey experiment."""
        ...
        for plan in self.plans:
            try:
                ...
                if action == 'move':
                    ...
                    await self._do_rest()
                elif action == 'share_observation':
                    target_id = plan.get('target', '')
                    if target_id:
                        await self._do_share_observation(target_id)
                elif action == 'give':
                    target_id = plan.get('target', '')
    ...
    async def _do_share_observation(self, target_id: str):
        """Share all observation data with a nearby agent."""
        try:
            new_pairs = await self.controller.run_environment(
                "survey", "share_observations", self.agent_id, target_id
            )
            await self._update_status(self.agent_id, {"energy": -3, "happiness": 5, "socialization": 5})
            logger.info(f'Agent {self.agent_id} shared observations with {target_id} ({new_pairs} new attr-region pairs)')
        except Exception as e:
            logger.warning(f"Agent {self.agent_id}: share_observation failed: {e}")

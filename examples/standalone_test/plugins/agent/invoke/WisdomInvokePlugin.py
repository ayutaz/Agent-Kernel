...
class WisdomInvokePlugin(InvokePlugin):
    """Execute planned actions with knowledge sharing for Wisdom of Crowds experiment."""
        ...
        for plan in self.plans:
            try:
                ...
                if action == 'move':
                    ...
                    target_id = plan.get('target', '')
                    if target_id:
                        await self._do_share_knowledge(target_id)
                else:
    ...
    async def _do_share_knowledge(self, target_id: str):
        """Share all known knowledge fragments with a nearby agent."""
        try:
            my_knowledge = await self.controller.run_environment(
                "knowledge", "get_agent_knowledge", self.agent_id
            )
            if not my_knowledge:
                logger.info(f"Agent {self.agent_id} has no knowledge to share with {target_id}")
                return

            transferred = await self.controller.run_environment(
                "knowledge", "share_knowledge", self.agent_id, target_id, list(my_knowledge)
            )

            await self._update_status(self.agent_id, {"energy": -3, "happiness": 5, "socialization": 5})
            logger.info(f'Agent {self.agent_id} shared knowledge with {target_id} ({transferred} new fragments)')
        except Exception as e:
            logger.warning(f"Agent {self.agent_id}: share_knowledge failed: {e}")

...
- **集合知スケーリング実験** — エージェント数と集団パフォーマンスの関係を測定するWisdom of Crowds実験フレームワークを内蔵。知識断片の分散探索・共有タスク（wisdom）と、多様な専門家による避難候補地評価タスク（survey）の2シナリオで、スケーリング則を検証可能。
...
### 3. 集合知スケーリング実験（Wisdom of Crowds — 知識断片探索）
...
### 4. 災害対応サーベイ実験（Wisdom of Crowds — 避難候補地評価）

地震後の5つの避難候補地を5属性（構造安全性・水/衛生・医療アクセス・物資経路・収容力）で評価するシナリオです。各専門家は自分の専門分野の**2属性のみ**観測可能で、5職業がリング構造で属性をカバーします。個体では原理的に正確な評価ができないため、多様な専門家チームの情報統合が集団推定精度向上の鍵となります。
...
- [集合知スケーリング実験（知識断片探索）](#3-集合知スケーリング実験wisdom-of-crowds--知識断片探索)
- [災害対応サーベイ実験](#4-災害対応サーベイ実験wisdom-of-crowds--避難候補地評価)
- [📂 プロジェクト構成](#-プロジェクト構成)
- [🎓 引用](#-引用)
- [🤝 コントリビューター](#-コントリビューター)
- [📜 ライセンス](#-ライセンス)
...
│       └── run_wisdom_experiment.py  # 集合知スケーリング実験ランナー（wisdom / survey）
│
├── society-panel/
